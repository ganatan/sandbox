...
class PersonControllerTest {
	...
	void shouldReturnAllPersons() throws Exception {
		...
		mockMvc.perform(get("/persons")).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(2))
	...
	void shouldReturnPersonById() throws Exception {
		...
		mockMvc.perform(get("/persons/1")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1))
	...
	void shouldCreatePerson() throws Exception {
		...
		mockMvc.perform(post("/persons").contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"Chris Evans\"}"))
	...
	void shouldUpdateExistingPerson() throws Exception {
		...
		mockMvc.perform(
				put("/persons/1").contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"Chris Hemsworth\"}"))
	...
	void shouldReturnNotFoundOnUpdateMissingPerson() throws Exception {
		...
		mockMvc.perform(put("/persons/99").contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"Unknown\"}"))
	...
	void shouldReturnNotFoundOnDeleteMissingPerson() throws Exception {
		...
		mockMvc.perform(delete("/persons/99")).andExpect(status().isNotFound());
	}
}
